<form method="POST" action="<?= $page ? '/admin/pages/update/'.$page->id : '/admin/pages/store' ?>" class="bg-white rounded-lg shadow p-6 max-w-4xl">
    ...
    <div class="mt-5"><label class="block text-sm font-medium text-gray-700 mb-1">Content (HTML)</label><textarea name="content" rows="15" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-bronze outline-none resize-y font-mono"><?= esc($page->content ?? '') ?></textarea></div>
