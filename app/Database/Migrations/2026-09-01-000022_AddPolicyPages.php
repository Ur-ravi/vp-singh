...
/**
 * Migration: Add comprehensive policy pages and footer menu items.
 *
 * This migration:
 * 1. Updates 3 existing policy pages with comprehensive content
 * 2. Inserts 2 new policy pages (Refund & Cancellation, Cookie)
 * 3. Adds 2 new footer menu items
 *
 * Safe to run multiple times (idempotent).
 */
class AddPolicyPages extends Migration
{
    public function up()
    {
        $db = $this->db;
        ...
        // ============================================================
        // 1. UPDATE existing policy pages with comprehensive content
        // ============================================================
...
<p><em>Effective Date: January 2026</em></p>';
        ...
        $db->table('pages')->where('slug', 'terms-conditions')->update([
            'content'         => $content,
...
<p><em>Effective Date: January 2026</em></p>';
        ...
        $db->table('pages')->where('slug', 'legal-disclaimer')->update([
            'content'         => $content,
        ...
        // ============================================================
        // 2. INSERT new policy pages (skip if slug already exists)
        // ============================================================

        $existing = $db->table('pages')->whereIn('slug', [
        ...
        if (!in_array('refund-cancellation-policy', $existingSlugs)) {
            $newPages[] = [
        ...
        if (!in_array('cookie-policy', $existingSlugs)) {
            $newPages[] = [
                'title'           => 'Cookie Policy',
                'slug'            => 'cookie-policy',
        ...
        if (!empty($newPages)) {
            $db->table('pages')->insertBatch($newPages);
        }

        // ============================================================
        // 3. INSERT footer menu items (skip if URL already exists)
        // ============================================================

        $footerMenu = $db->table('menus')->where('location', 'footer')->get()->getRow();

        if ($footerMenu) {
            $existingItems = $db->table('menu_items')
                ->where('menu_id', $footerMenu->id)
                ->whereIn('url', ['/refund-cancellation-policy', '/cookie-policy'])
                ->get()
                ->getResult();

            $existingUrls = array_column($existingItems, 'url');
            ...
            if (!in_array('/refund-cancellation-policy', $existingUrls)) {
                $maxSort++;
                $newMenuItems[] = [
            ...
            if (!in_array('/cookie-policy', $existingUrls)) {
                $maxSort++;
                $newMenuItems[] = [
                    'menu_id'     => $footerMenu->id,
                    'parent_id'   => null,
            ...
            if (!empty($newMenuItems)) {
                $db->table('menu_items')->insertBatch($newMenuItems);
            }
        }
    }
    ...
    public function down()
    {
        ...
        // Remove new policy pages
        $db->table('pages')->whereIn('slug', [
            'refund-cancellation-policy',
            'cookie-policy',
        ])->delete();

        // Remove new footer menu items
        $db->table('menu_items')->whereIn('url', [
            '/refund-cancellation-policy',
            '/cookie-policy',
        ])->delete();
    }
    ...
    // ================================================================
    // Policy Content Methods
    // ================================================================

    private function getRefundPolicyContent(): string
    {
        return <<<'HTML'
<p>This Refund and Cancellation Policy outlines the terms governing refunds and cancellations for consultation services provided by V P Singh Advocate.</p>
...
HTML;
    ...
    {
        return <<<'HTML'
<p>This Cookie Policy explains how V P Singh Advocate ("we", "our", or "us") uses cookies and similar technologies when you visit our website. This policy helps you understand what cookies are, how we use them, and how you can manage your cookie preferences.</p>
